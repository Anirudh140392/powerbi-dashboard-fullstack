...
const newGetReviews = `export const getReviews = async (req, res) => {
    try {
        const {
...
const startIndex = content.indexOf('export const getReviews = async (req, res) => {');
const endOfGetReviews = content.indexOf('export const searchReviews = async (req, res) => {');
if (startIndex !== -1 && endOfGetReviews !== -1) {
    // Remove the comment block preceding searchReviews as well if needed, but it's fine.
    const before = content.slice(0, startIndex);
    const after = content.slice(endOfGetReviews);
    writeFileSync(file, before + newGetReviews + '\n\n' + after);
    console.log('Replaced getReviews!');
} else {
    console.log('Could not find getReviews boundaries');
}
