...
async function test() {
    ...
    const masterPlatformFiltStr = '';
    const snapshotDateFilter = `AND snapshot_date >= subtractMonths(today(), 6)`;
    const recentFilter = `AND r.review_date >= subtractMonths(today(), 3)`;
    const priorFilter = `AND r.review_date >= subtractMonths(today(), 6) AND r.review_date < subtractMonths(today(), 3)`;
    const sentimentCategoryFilters = [`r.sentiment_category ILIKE {sentimentCategory:String}`];
