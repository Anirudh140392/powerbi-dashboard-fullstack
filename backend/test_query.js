...
async function test() {
  await clickhouse.query({ query: 'SELECT 1' }); // check connection
  ...
  const query = `
        ...
        comp_gainer AS (
            SELECT city, platform, category, argMax(Product, ms_gap) AS comp_sku, MAX(ms_gap) AS max_ms_gap
            FROM sku_ms_gap
            WHERE Comp_flag IN (1, '1') AND ms_gap > 0
            GROUP BY city, platform, category
        )
        SELECT * FROM comp_gainer LIMIT 10;
  `;
  try {
