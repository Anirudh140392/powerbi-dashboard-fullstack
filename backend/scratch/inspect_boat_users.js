...
async function check() {
    try {
        ...
        const users = await queryAdminDB("SELECT user_email, user_name, toString(db_id) as db_id, access, last_login FROM tb_user");
        
        console.log("All rows in tb_user that match 'boat':");
        users.forEach(user => {
            let finalDbName = 'N/A';
