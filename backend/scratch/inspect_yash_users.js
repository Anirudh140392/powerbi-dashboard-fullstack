...
async function check() {
    try {
        ...
        const users = await queryAdminDB("SELECT user_email, user_name, toString(db_id) as db_id, access, last_login FROM tb_user WHERE user_name LIKE '%Yash%' OR user_email LIKE '%yash%'");
        
        console.log("All rows for Yash in tb_user:");
        users.forEach(user => {
            let finalDbName = 'N/A';
            ...
            console.log(`Email: ${user.user_email}, Name: ${user.user_name}, dbName: ${finalDbName}, db_id: ${user.db_id}, access: ${user.access}, last_login: ${user.last_login}`);
        });
    } catch (err) {
