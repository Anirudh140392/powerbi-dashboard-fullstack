...
async function test() {
    ...
    for (const db of dbs) {
        ...
        const q = `ALTER TABLE admin_master.tb_database UPDATE Internal_kam = '${str}' WHERE db_id = ${db.db_id} SETTINGS mutations_sync = 1`;
        await queryAdminDB(q);
        console.log("Updated db_id sync:", db.db_id);
    }
}
