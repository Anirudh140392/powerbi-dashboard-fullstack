...
async function verifyFilters() {
    ...
    try {
        ...
        const [locations] = await connection.execute('SELECT DISTINCT location_name FROM rb_location ORDER BY location_name ASC LIMIT 5');
        console.log('Locations:', locations.map(l => l.location_name));
