...
async function runFilterLogic(reqQuery) {
    ...
    console.log("Running minMaxQuery:\n", minMaxQuery);
...
async function main() {
    await dbStorage.run('emami', async () => {
        console.log("--- Querying with empty params ---");
        await runFilterLogic({});

        console.log("--- Querying with Girish / default KAM ---");
        await runFilterLogic({ kam: 'Girish' });
    });
}
