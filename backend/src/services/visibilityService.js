        ...
        return await getCachedOrCompute(cacheKey, async () => {
            try {
                ...
                SELECT 
                    ...
                    ROUND(countIf(${brandSOSCondition} AND spons = 1) * 100.0 / nullIf(count(), 0), 2) AS sponsored_sos,
                    ROUND(countIf(${brandSOSCondition} AND spons != 1) * 100.0 / nullIf(count(), 0), 2) AS organic_sos,
                    ROUND(countIf(${brandSOSCondition} AND (DATE < '2025-01-01' OR spons = 1)) * 100.0 / nullIf(count(), 0), 2) AS display_sos
                FROM rb_kw_olap
                WHERE DATE BETWEEN '${dateFrom}' AND '${dateTo}'
                  AND POSITION < 11
                  AND ${platformCondition}
    ...
    async getVisibilityCompetition(filters = {}) {
        ...
        return await getCachedOrCompute(cacheKey, async () => {
            ...
            `;
                ...
                SELECT 
                    brand_name_th as brand_name,
                    ROUND(countIf(DATE BETWEEN '${dateFrom}' AND '${dateTo}') * 100.0 / ${currentVolume}, 2) AS current_overall_sos,
                    ...
                    ROUND(countIf(DATE BETWEEN '${prevDateFrom}' AND '${prevDateTo}') * 100.0 / ${prevVolume}, 2) AS prev_overall_sos,
            ...
            `;
                ...
                SELECT 
                    ...
                    brand_name_th as brand_name,
                    ROUND(countIf(overall = 1) * 100.0 / nullIf(count(*), 0), 2) AS overall_sos,
                    ROUND(countIf(spons = 1) * 100.0 / nullIf(count(*), 0), 2) AS sponsored_sos,
                    ROUND(countIf(organic = 1) * 100.0 / nullIf(count(*), 0), 2) AS organic_sos,
