        ...
        try {
            ...
            SELECT
                ...
                AVG(CASE WHEN p.DATE BETWEEN '${startDate}' AND '${endDate}' 
                         AND toFloat64(p.MRP) > 0 
                    THEN ((toFloat64(p.MRP) - toFloat64(p.Selling_Price)) / toFloat64(p.MRP)) * 100 
                    ELSE NULL END) AS discount_curr,
                
                SUM(CASE WHEN p.DATE BETWEEN '${startDate}' AND '${endDate}' 
                         AND toFloat64(p.MRP) > 0 
                    THEN ((toFloat64(p.MRP) - toFloat64(p.Selling_Price)) / toFloat64(p.MRP)) * toFloat64(p.Sales) 
                    ELSE 0 END) / 
                NULLIF(SUM(CASE WHEN p.DATE BETWEEN '${startDate}' AND '${endDate}' THEN toFloat64(p.Sales) ELSE 0 END), 0) * 100 AS weighted_discount_curr,
                
                AVG(CASE WHEN p.DATE BETWEEN '${startDate}' AND '${endDate}' 
                         AND toFloat64(extract(p.Weight, '^[0-9.]+')) > 0 
                    THEN toFloat64(p.Selling_Price) / toFloat64(extract(p.Weight, '^[0-9.]+')) 
                    ELSE NULL END) AS price_per_unit_curr,
                
                AVG(CASE WHEN p.DATE BETWEEN '${startDate}' AND '${endDate}' 
                         AND toFloat64(p.MRP) > 0 
                    THEN toFloat64(p.Selling_Price) / toFloat64(p.MRP) 
                    ELSE NULL END) AS rpi_curr,
                
                AVG(CASE WHEN p.DATE BETWEEN '${startDate}' AND '${endDate}' 
                    THEN toFloat64(p.Selling_Price) 
                ...
                SUM(CASE WHEN p.DATE BETWEEN '${compareStartDate}' AND '${compareEndDate}' 
                    ...
                    THEN ((toFloat64(p.MRP) - toFloat64(p.Selling_Price)) / toFloat64(p.MRP)) * toFloat64(p.Sales) 
                    ELSE 0 END) / 
                NULLIF(SUM(CASE WHEN p.DATE BETWEEN '${compareStartDate}' AND '${compareEndDate}' THEN toFloat64(p.Sales) ELSE 0 END), 0) * 100 AS weighted_discount_prev,
