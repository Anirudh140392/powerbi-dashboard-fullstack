...
/**
 * Send KAM alert/report emails to recipients using BCC so recipients cannot see each other.
 *
...
const sendAlertEmailToRecipients = async (transporter, { fromEmail, fromName, recipientEmailsStr, subject, text, html }) => {
    ...
    try {
        ...
        console.log(`[KamAlertCron] 📧 KAM email delivered via BCC to ${recipients.length} recipient(s) (${recipients.join(', ')}). Message ID: ${info.messageId}`);
        return true;
    } catch (sendErr) {
        console.error(`[KamAlertCron] Failed to send KAM BCC email to recipients (${recipients.join(', ')}):`, sendErr.message);
        ...
        for (const recipient of recipients) {
            ...
            try {
                ...
                console.log(`[KamAlertCron] 📧 Fallback KAM BCC email delivered to ${recipient}. Message ID: ${info.messageId}`);
                successCount++;
            } catch (err) {
                console.error(`[KamAlertCron] Fallback KAM BCC send failed for ${recipient}:`, err.message);
            }
        }
