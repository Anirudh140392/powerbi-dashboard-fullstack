...
async function verifySalesQuery() {
    ...
    try {
        ...
        console.log(`Querying from ${widerStartDate} to ${endDate}`);

        // Simplified query to just test the JOIN logic on a smaller dataset
        const query = `
            WITH daily_agg AS(
                SELECT 
                ...
                AND Platform = 'Zepto' -- Filter to reduce data load
