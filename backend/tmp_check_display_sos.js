...
async function test() {
    try {
        console.log("Checking dates in rb_kw_olap...");
        const dates = await queryClickHouse("SELECT min(created_on) as min_date, max(created_on) as max_date FROM rb_kw_olap");
