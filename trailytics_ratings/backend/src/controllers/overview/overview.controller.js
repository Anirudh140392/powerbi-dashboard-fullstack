...
export const getCategoryHealth = async (req, res) => {
    try {
        ...
        const sql = `
            ...
            sku_category_map AS (
                SELECT web_pid, sku_code, coalesce(sku_code, web_pid) AS canonical_sku,
            ...
            cat_sku_counts AS (
                ...
                WHERE 1=1 ${snapCatFiltStr.replace("coalesce(nullIf(ls.category, ''), nullIf(mp.category, ''))", "category")}
                GROUP BY category
            ),
            cat_reviews AS (
                SELECT
                ...
                WHERE r.company_id = {companyId:String} ${competitorFilter} ${platformFiltStr} ${reviewPriceFiltStr} ${catFiltStr} ${currentScopeFilter}
