...
if __name__ == "__main__":
    ...
    sigs = [
        "app.post('/api/ml/jobs/spawn'",
        "app.get('/api/ml/jobs'",
        "app.post('/api/ml/master-enrich'"
    ]
    
    results = extract_routes(file_path, sigs)
    
    names = {
        "app.post('/api/ml/jobs/spawn'": "spawnMlJob",
        "app.get('/api/ml/jobs'": "getMlJobs",
        "app.post('/api/ml/master-enrich'": "masterEnrich"
    }

    with open('src/controllers/ml/ml.controller.js', 'w') as out:
        out.write("import pool from '../../config/db.js';\n\n")
        # Ensure spawnJob utilities are imported in ml.controller.js as they might be used
