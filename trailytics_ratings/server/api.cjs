...
app.use((req, res, next) => {
    ...
    res.json = (body) => {
        try {
            ...
            RESPONSE_CACHE.set(key, { body, expires: Date.now() + RESPONSE_CACHE_TTL_MS });
            res.set('Cache-Control', `public, max-age=60, stale-while-revalidate=300`);
...
app.post('/api/auth/mfa/verify', async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'MFA verification failed', details: error.message, stack: error.stack });
    }
});
    ...
    try {
        ...
        const { rows } = await pool.query(`
            WITH latest AS (
                ...
                WHERE ${where}
                ORDER BY ps.company_id, ps.platform, ps.web_pid, ps.snapshot_date DESC
            )
            SELECT
    ...
    try {
        ...
        const [agg, sample] = await Promise.all([
            pool.query(`
                SELECT brand,
                       COUNT(*) AS total,
                       COUNT(*) FILTER (WHERE is_favorable) AS favorable,
                       COUNT(*) FILTER (WHERE sentiment = 'Negative' AND NOT is_favorable) AS unfavorable,
                       COUNT(*) FILTER (WHERE sentiment = 'Neutral' AND NOT is_favorable) AS neutral
    ...
    try {
        ...
        const [categories, materials, brands, platforms, paretoStatuses] = await Promise.all([
            ...
            pool.query(`
                ...
                ORDER BY brand
            `, params),
            pool.query(`SELECT DISTINCT platform FROM ratings.reviews r WHERE r.company_id = $1 ${competitorFilter} ORDER BY platform`, params),
            pool.query(`
        ...
        const sql = `
            ...
            cat_products AS (
                ...
                JOIN (
                    ...
                    FROM ratings.product_snapshots ps
                    WHERE ps.company_id = $1
                    ORDER BY ps.company_id, ps.platform, ps.web_pid, ps.snapshot_date DESC, ps.created_at DESC
...
app.get('/api/ratings/executive-health', async (req, res) => {
    try {
        ...
        res.json({
            pareto: formatBucket('Pareto', buckets['Pareto']),
            nonPareto: formatBucket('Non-Pareto', buckets['Non-Pareto']),
            npd: formatBucket('NPD', buckets['NPD']),
...
app.get('/api/data-lake/export', async (req, res) => {
    try {
        ...
        const sql = `
            WITH latest_snapshots AS (
                SELECT DISTINCT ON (company_id, platform, web_pid) *
                FROM ratings.product_snapshots
                WHERE company_id = $1
                ORDER BY company_id, platform, web_pid, snapshot_date DESC, created_at DESC NULLS LAST
            )
            SELECT 
                r.web_pid as "ID",
