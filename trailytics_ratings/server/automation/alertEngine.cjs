...
async function evaluateRule(pool, rule) {
    ...
    if (avgDays) {
        ...
        // Includes category + pareto_status so the digest can group SKUs
        // without re-querying at render time.
        sql = `
            WITH scoped AS (
            ...
            SELECT c.web_pid, c.platform, c.product_name, c.rating_count, c.snapshot_date,
                   ...
                   (
                       ...
                       WHERE s.company_id = c2.company_id
                         ...
                         AND s.snapshot_date >= c.snapshot_date - INTERVAL '${avgDays} days'
