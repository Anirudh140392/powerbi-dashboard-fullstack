...
import { MfaEnrolment } from './auth/MfaEnrolment';
...
type Step =
    ...
    | { kind: 'enrol'; challengeToken: string; email: string }
...
const LoginPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (result.status === 'success') {
            // Legacy single-step path — session already persisted by AuthContext.
            return;
        }
        ...
        if (result.status === 'enrol') {
            setStep({ kind: 'enrol', challengeToken: result.challengeToken, email: result.email });
            return;
        }
    };
                ...
                <div className="backdrop-blur-xl bg-white/[0.08] border border-white/[0.12] rounded-3xl shadow-2xl p-8">
                    ...
                    {step.kind === 'verify' && (
                        <MfaVerify
                            ...
                            onCancel={() => { setStep({ kind: 'password' }); setPassword(''); }}
                        />
                    )}
                    ...
                    {step.kind === 'enrol' && (
                        <MfaEnrolment
                            challengeToken={step.challengeToken}
                            email={step.email}
                            onSuccess={(backupCodes) => setStep({ kind: 'backup', backupCodes })}
                        />
                    )}
                    ...
                    {step.kind === 'backup' && (
                        <MfaBackupCodes
                            ...
                            onContinue={() => {
                                // Session is already persisted from /enrol/confirm — refresh
                                // the AuthContext state so AppContent flips to the dashboard.
                                refreshSession();
                            }}
                        />
                    )}
