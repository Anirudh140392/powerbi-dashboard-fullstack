...
// Apply ML classifications from ratings.reviews_ml_audit -> ratings.reviews.
// Idempotent (fills NULLs only) + resumable + heartbeats via runNodeScript.
// Generous cap for the first run that clears the historical backlog; after that
// daily runs only touch newly-classified rows and finish quickly.
const { runApplyMlAudit } = proxyActivities<typeof activities>({
...
export async function dailyPipelineWorkflow(input: PipelineInput): Promise<{
  ...
  // --- Stage 2b: apply ML classifications from the audit table onto reviews ---
  // Without this the models classify into ratings.reviews_ml_audit but the
  // dashboard (which reads ratings.reviews) never sees them. Non-fatal.
  try {
    await runApplyMlAudit({ companyId: input.companyId });
